<x-app-layout>
  <style>
    ...
    <form action="{{route('pegawai.mydoc.carimydoc')}}" method="GET" enctype="multipart/form-data">
    ...
    <table class="table-fixed min-w-full divide-y divide-green-200 border-gray-500 border-2 md:overflow-scroll mt-7">
        <thead class="bg-green-50 border-b border-green-500">
            <tr class="">
                ...
                <th class="px-2 py-3 w-1/6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode</th>
        ...
        <tr class="hover:bg-gray-200 divide-x divide-green-100">
              ...
              @if (($item->status_id) == '2')
                ...
                @else
                  <div class="tooltip">
                    ...
                    <svg class="inline-block bg-yellow-500 rounded-full h-6 w-6 items-center justify-center py-1 px-1 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <td class="px-2 py-4">{{$item->kode}}</td>
            <td class="px-2 py-4">{{$item->created_at->format("d F, Y H:i")}}</td>
            <td class="px-2 py-4">
                @if (($item->status_id) == '1')
                <a href="{{route('pegawai.mydoc.edit',$item->id)}}">
                   <div class="tooltip">
                ...
                <a class="px-2" href="{{route('pegawai.delete', $item->id)}}" onclick="return confirm('Apakah anda yakin ingin menghapus data ini?')">
                    <div class="tooltip">
                ...
                <a href="{{route('pegawai.mydoc.show',$item->id)}}">
                     <div class="tooltip">
